...
List<TimeValueClass> timedata = [
  TimeValueClass(id:localid,hour: "00", min: "10", sec: "00", label: 'Boil egg'),
];
void main() async{
  ...
  database = openDatabase(
    join(await getDatabasesPath(), "timerdb.db"),
    version: 1,
...
Future<void> insertTime(TimeValueClass timeValueClassobj) async {
  print("Insert Data *******");
  final localDB = await database;
    await localDB.insert(
    'timertable',
    timeValueClassobj.timeValueClassMap(),
    conflictAlgorithm: ConflictAlgorithm.replace,
  );
  print("Inserted data successfully");
  print(await getMOdelClasstimeData());
}

Future<List<TimeValueClass>> getMOdelClasstimeData() async{
  final localDB = await database;

  List<Map<dynamic,dynamic>> data = await localDB.query(
    "timertable",
  );
  return List.generate(data.length, (index){
    return TimeValueClass(
        id:timedata[localid].id,
        hour: timedata[localid].hour,
        min: timedata[localid].min, 
        sec: timedata[localid].sec, label:
        timedata[localid].label);
  });
}
Future updateList() async {
    var fetchedData = await gettimedateFromDatabase() ;
...
Future<List> gettimedateFromDatabase() async {
  final localDB = await database;
  List timedata = await localDB.query("timertable");
  return timedata;
}


// Future<void> deleteCardInDatabase(TimeValueClass timeValueClassobj) async {
//   final localDB = await database;
//   await localDB.delete(
//     'ToDoTable2',
//     where: "id = ?",
//     whereArgs: [timeValueClassobj.id],
//   );
// }
...
//int currentMin = 0;
                                ...
                                GestureDetector(
                                  onTap: () {
                                    ...
                                    TimeValueClass timeValueClassobj1 = TimeValueClass(
                                      ...
                                      updatedatetimeInDatabase(TimeValueClass(
                                        id:localid,
                                      hour: _hourcontroller.text,
                                      min: _mincontroller.text,
                                      sec: _seccontroller.text,
  ...
  Future<void> updatedatetimeInDatabase(TimeValueClass timeValueClassobj) async {
  final localDB = await database;
  Map newCard = await timeValueClassobj.timeValueClassMap();
...
class TimeValueClass{
  ...
  Map<String, dynamic> timeValueClassMap(){
